/* ==========================================
   鲁菜 · Shandong
   咸鲜为本，火候见功
   ========================================== */
window.RECIPES = window.RECIPES || {};
window.RECIPES.lu = [
  {
    
    name: "糖醋鲤鱼",
    id: "lu-tangculiyu",
    image: "assets/img/lu-tangculiyu.webp",
    desc: "金鳞酥脆，酸甜适口，鲁菜宴席的经典开席大菜。",
    time: "40 分钟",
    difficulty: "中等",
    serves: "3-4 人",
    ingredients: [
      { name: "鲜活鲤鱼", amount: 750, unit: "克（整条）" },
      { name: "葱", amount: 2, unit: "根" },
      { name: "姜", amount: 10, unit: "克" },
      { name: "蒜", amount: 4, unit: "瓣" }
    ],
    seasonings: [
      { name: "醋", amount: 50, unit: "毫升" },
      { name: "白糖", amount: 60, unit: "克" },
      { name: "料酒", amount: 2, unit: "汤匙" },
      { name: "生抽", amount: 1, unit: "汤匙" },
      { name: "淀粉", amount: 80, unit: "克" },
      { name: "食用油", amount: 500, unit: "毫升" }
    ],
    steps: [
      "鲤鱼去鳞去鳃去内脏，剞十字花刀，用料酒与盐略腌。",
      "鱼身拍匀干淀粉，油温六成热下锅炸至定型，捞出复炸至酥脆。",
      "锅留底油，爆香葱姜蒜，烹醋炒香。",
      "加白糖、生抽与少量清水烧开，勾薄芡收至浓稠。",
      "将糖醋汁浇在炸好的鲤鱼上即成。"
    ],
    tips: "先定油温复炸提酥，糖醋汁要热油爆出酸香。"
  },
  {
    
    name: "九转大肠",
    id: "lu-jiuzhuan",
    image: "assets/img/lu-jiuzhuan.webp",
    desc: "肥肠软糯，咸甜酸辣香五味俱全，鲁菜名品。",
    time: "90 分钟",
    difficulty: "较难",
    serves: "3-4 人",
    ingredients: [
      { name: "猪大肠", amount: 800, unit: "克" },
      { name: "葱", amount: 2, unit: "根" },
      { name: "姜", amount: 10, unit: "克" },
      { name: "蒜", amount: 3, unit: "瓣" }
    ],
    seasonings: [
      { name: "白糖", amount: 80, unit: "克" },
      { name: "醋", amount: 40, unit: "毫升" },
      { name: "料酒", amount: 3, unit: "汤匙" },
      { name: "生抽", amount: 2, unit: "汤匙" },
      { name: "老抽", amount: 1, unit: "茶匙" },
      { name: "花椒", amount: 3, unit: "克" },
      { name: "肉桂", amount: 1, unit: "小块" }
    ],
    steps: [
      "大肠反复翻洗去异味，冷水下锅加姜葱焯透切段。",
      "热锅爆香葱姜蒜与花椒，下大肠煸炒出油。",
      "烹料酒与醋，加生抽、老抽、足量糖与清水。",
      "小火慢烧约 40 分钟至软糯入味，中间加肉桂增香。",
      "大火收汁，边收边翻，汁浓亮油裹匀即成。"
    ],
    tips: "糖量要足，咸甜酸辣香五味平衡才有九转风味。"
  },
  {
    
    name: "葱烧海参",
    id: "lu-congshaohaishen",
    image: "assets/img/lu-congshaohaishen.webp",
    desc: "海参软糯弹润，葱香浓郁，胶东名贵宴客硬菜。",
    time: "60 分钟",
    difficulty: "较难",
    serves: "3-4 人",
    ingredients: [
      { name: "水发海参", amount: 4, unit: "只" },
      { name: "京葱", amount: 4, unit: "根" },
      { name: "姜", amount: 6, unit: "片" }
    ],
    seasonings: [
      { name: "高汤", amount: 300, unit: "毫升" },
      { name: "蚝油", amount: 2, unit: "汤匙" },
      { name: "生抽", amount: 1, unit: "汤匙" },
      { name: "老抽", amount: 0.5, unit: "汤匙" },
      { name: "料酒", amount: 1, unit: "汤匙" },
      { name: "白糖", amount: 1, unit: "茶匙" }
    ],
    steps: [
      "海参洗净改刀，姜葱水焯透去腥沥干。",
      "热油将整段京葱煎至金黄出葱油。",
      "下海参略煎，烹料酒，注入高汤。",
      "加蚝油、生抽、老抽与糖，小火煨烧 20 分钟。",
      "待汤浓味厚，勾薄芡，摆上葱段即成。"
    ],
    tips: "葱油是灵魂，小火慢煨让海参充分吸味。"
  },
  {
    
    name: "锅塌豆腐",
    id: "lu-guotadoufu",
    image: "assets/img/lu-guotadoufu.webp",
    desc: "外酥里嫩，汤汁鲜香，鲁菜锅塌技法的代表菜。",
    time: "30 分钟",
    difficulty: "简单",
    serves: "2-3 人",
    ingredients: [
      { name: "北豆腐", amount: 400, unit: "克" },
      { name: "鸡蛋", amount: 2, unit: "个" },
      { name: "猪肉末", amount: 60, unit: "克" },
      { name: "葱", amount: 1, unit: "根" },
      { name: "姜", amount: 5, unit: "克" }
    ],
    seasonings: [
      { name: "高汤", amount: 200, unit: "毫升" },
      { name: "生抽", amount: 1, unit: "汤匙" },
      { name: "料酒", amount: 1, unit: "汤匙" },
      { name: "盐", amount: 3, unit: "克" },
      { name: "淀粉", amount: 20, unit: "克" }
    ],
    steps: [
      "豆腐切片，裹蛋液上薄粉。",
      "平底锅热油，将豆腐煎至两面金黄。",
      "下肉末与葱姜煸香，烹料酒。",
      "注高汤加生抽盐，小火塌煮使汤汁收浓。",
      "汤汁裹满豆腐即可出锅。"
    ],
    tips: "小火慢塌出香，蛋香与豆香交融。"
  },
  {
    
    name: "油爆双脆",
    id: "lu-youbaoshuangcui",
    image: "assets/img/lu-youbaoshuangcui.webp",
    desc: "鸡胗与肚尖入口爽脆，旺火快炒，考验火候的爆款菜。",
    time: "25 分钟",
    difficulty: "较难",
    serves: "2-3 人",
    ingredients: [
      { name: "鸡胗", amount: 200, unit: "克" },
      { name: "猪肚尖", amount: 200, unit: "克" },
      { name: "青红椒", amount: 40, unit: "克" },
      { name: "葱姜蒜", amount: "适量", unit: "" }
    ],
    seasonings: [
      { name: "料酒", amount: 2, unit: "汤匙" },
      { name: "生抽", amount: 1, unit: "汤匙" },
      { name: "香油", amount: 1, unit: "茶匙" },
      { name: "盐", amount: 3, unit: "克" },
      { name: "淀粉", amount: 10, unit: "克" }
    ],
    steps: [
      "鸡胗、肚尖剞花刀切成麦穗状，加盐抓匀。",
      "调好碗芡（料酒、生抽、盐、淀粉、清水）。",
      "油锅烧至大热，下双脆猛火滑断生即沥出。",
      "锅留底油爆香小料，回锅双脆，冲入碗芡。",
      "旺火快翻均匀，淋香油迅速出锅。"
    ],
    tips: "全程旺火快抢，动作利落才保双脆爽口。"
  },
  {
    
    name: "四喜丸子",
    id: "lu-sixiwanzi",
    image: "assets/img/lu-sixiwanzi.webp",
    desc: "个大肉圆，软糯咸香，寓意福禄寿喜的宴席吉祥菜。",
    time: "90 分钟",
    difficulty: "中等",
    serves: "4-6 人",
    ingredients: [
      { name: "猪前腿肉", amount: 600, unit: "克" },
      { name: "马蹄", amount: 80, unit: "克" },
      { name: "香菇", amount: 4, unit: "朵" },
      { name: "鸡蛋", amount: 1, unit: "个" },
      { name: "青菜心", amount: 100, unit: "克" }
    ],
    seasonings: [
      { name: "生抽", amount: 2, unit: "汤匙" },
      { name: "老抽", amount: 1, unit: "汤匙" },
      { name: "料酒", amount: 2, unit: "汤匙" },
      { name: "白糖", amount: 1, unit: "汤匙" },
      { name: "淀粉", amount: 30, unit: "克" }
    ],
    steps: [
      "肉切细粒，马蹄香菇切丁，加蛋液淀粉调入生抽料酒。",
      "顺同一方向搅打上劲，团成四个大丸子。",
      "热油炸至外表金黄定型。",
      "丸子入砂锅，加老抽、糖与开水小火炖 50 分钟。",
      "收汁勾芡，摆上烫熟的青菜心围边。"
    ],
    tips: "搅打上劲肉才弹，小火久炖才软糯入味。"
  },
  {
    
    name: "爆炒腰花",
    id: "lu-baochaoyaohua",
    image: "assets/img/lu-baochaoyaohua.webp",
    desc: "腰花脆嫩无膻，酱香浓郁，鲁菜经典下饭菜。",
    time: "30 分钟",
    difficulty: "中等",
    serves: "2-3 人",
    ingredients: [
      { name: "猪腰", amount: 400, unit: "克" },
      { name: "洋葱", amount: 60, unit: "克" },
      { name: "青椒", amount: 40, unit: "克" },
      { name: "木耳", amount: 30, unit: "克" },
      { name: "葱姜蒜", amount: "适量", unit: "" }
    ],
    seasonings: [
      { name: "生抽", amount: 1, unit: "汤匙" },
      { name: "老抽", amount: 0.5, unit: "汤匙" },
      { name: "料酒", amount: 2, unit: "汤匙" },
      { name: "醋", amount: 1, unit: "茶匙" },
      { name: "白糖", amount: 1, unit: "茶匙" },
      { name: "淀粉", amount: 10, unit: "克" }
    ],
    steps: [
      "猪腰对半片开，去净腰臊，剞麦穗花刀切块。",
      "腰花加料酒盐腌渍，入热油滑至断生沥出。",
      "锅留底油，爆香葱姜蒜，下洋葱青椒木耳翻炒。",
      "倒入腰花，烹入调好的碗芡（酱醋糖淀粉水）。",
      "旺火快翻裹匀味汁，立即出锅。"
    ],
    tips: "去净腰臊是除腥关键，快炒收紧锁脆。"
  },
  {
    
    name: "糖醋里脊",
    id: "lu-tangculiji",
    image: "assets/img/lu-tangculiji.webp",
    desc: "色泽金黄，外酥里嫩，酸甜开胃的北方家庭硬菜。",
    time: "30 分钟",
    difficulty: "简单",
    serves: "3-4 人",
    ingredients: [
      { name: "猪里脊", amount: 400, unit: "克" },
      { name: "鸡蛋", amount: 1, unit: "个" },
      { name: "葱姜蒜", amount: "适量", unit: "" }
    ],
    seasonings: [
      { name: "白糖", amount: 60, unit: "克" },
      { name: "醋", amount: 40, unit: "毫升" },
      { name: "生抽", amount: 1, unit: "汤匙" },
      { name: "番茄酱", amount: 2, unit: "汤匙" },
      { name: "淀粉", amount: 60, unit: "克" },
      { name: "食用油", amount: 500, unit: "毫升" }
    ],
    steps: [
      "里脊切条，加盐料酒抓匀，裹蛋液拍干淀粉。",
      "油温六成热下锅炸至浅黄，复炸至金黄酥脆。",
      "调糖醋汁：糖、醋、番茄酱、生抽、水与淀粉。",
      "锅留底油熬至糖醋汁浓稠冒泡。",
      "下炸好的里脊，裹匀酱汁立即出锅。"
    ],
    tips: "复炸定型保脆，糖醋汁温度要够才裹得住。"
  },
  {
    
    name: "德州扒鸡",
    id: "lu-dezhoupaji",
    image: "assets/img/lu-dezhoupaji.webp",
    desc: "五香脱骨，肉烂形整，一抖即散的德州名鸡。",
    time: "150 分钟",
    difficulty: "较难",
    serves: "4-6 人",
    ingredients: [
      { name: "整鸡", amount: 1500, unit: "克（整只）" },
      { name: "蜂蜜", amount: 20, unit: "克" },
      { name: "姜", amount: 15, unit: "克" }
    ],
    seasonings: [
      { name: "八角", amount: 4, unit: "个" },
      { name: "桂皮", amount: 1, unit: "段" },
      { name: "花椒", amount: 4, unit: "克" },
      { name: "香叶", amount: 3, unit: "片" },
      { name: "酱油", amount: 3, unit: "汤匙" },
      { name: "白糖", amount: 20, unit: "克" }
    ],
    steps: [
      "整鸡洗净，翅别入口腔、腿交叉盘好定型。",
      "鸡身抹蜂蜜稍晾，入热油中炸至金黄上色。",
      "锅底铺葱姜与香辛料，放入整鸡。",
      "注入卤水加酱油糖盐，大火烧沸转小火。",
      "小火扒煮约 2 小时至脱骨，关火原汤浸凉。"
    ],
    tips: "定型要扎实，小火久扒才肉烂形整不散架。"
  },
  {
    
    name: "红烧大虾",
    id: "lu-hongshaodaxia",
    image: "assets/img/lu-hongshaodaxia.webp",
    desc: "虾肉弹嫩，酱香红亮，胶东海鲜家常硬菜。",
    time: "25 分钟",
    difficulty: "简单",
    serves: "3-4 人",
    ingredients: [
      { name: "渤海大虾", amount: 500, unit: "克" },
      { name: "姜", amount: 6, unit: "片" },
      { name: "葱", amount: 2, unit: "根" }
    ],
    seasonings: [
      { name: "白糖", amount: 15, unit: "克" },
      { name: "酱油", amount: 1, unit: "汤匙" },
      { name: "料酒", amount: 2, unit: "汤匙" },
      { name: "高汤", amount: 100, unit: "毫升" }
    ],
    steps: [
      "大虾剪须开背去虾线，冲洗沥干。",
      "热油爆香姜葱，下大虾煎至两面变红。",
      "加白糖炒出红油，烹料酒与酱油。",
      "注入高汤，小火焖烧至虾入味。",
      "大火收汁亮油，摆盘即食。"
    ],
    tips: "虾头虾膏炒出红油，色泽与鲜味俱佳。"
  }
];